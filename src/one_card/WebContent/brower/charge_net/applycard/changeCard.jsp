...
<script type="text/javascript">
	function check() {
		...
		var newc_id = document.getElementById("newc_id").value;
...
<body>
	<div style="margin: 10px auto; width: 600px; background-color: #F9F">
		...
		<form action="<%=path%>ChangeCardServlet?action=change_card"
			method="post">
			<div>
				请选择: <select id="selects" name="selects">
					<option value="c_id">原卡号</option>
					<option value="p_phone">手机号</option>
					<option value="p_idcard">证件号</option>


				</select> <input type="text" id="c_id" name="c_id" value=""> <input
			...
			<div>
				请输入新卡号： <input type="text" id="newc_id" name="newc_id" value="">
				<input type="button" id="quary_cid" name="quary_cid" value="换卡"
	...
	$(function(){
		...
		$("#read_card").on("click",function(){
			...
			$.ajax({
				...
				type : "post",
				dataType : "json",
				data : "action=read_card&c_id=" + c_id + "&selects=" + selects,
