							...
							<c:if test="${map.key ne '卡片管理'}">
								...
								<div class="menu_body" id="firstmenu">
									...
									<c:forEach items="${map.value}" var="v" varStatus="v">
										...
										<a> ${v} </a>
										<%-- 										<a> ${v.m_name} </a> --%>
										<%-- 										<c:if test="${val.m_state eq '启用'}"> --%>
										<%-- 											<a><input type="checkbox" name="${val.m_id}" --%>
										<%-- 												value="${val.m_id}">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${val.m_name}<br /> --%>
										<!-- 											</a> -->
									</c:forEach>
								</div>
