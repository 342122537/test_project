						...
						<c:forEach items="${card_messageList}" var="u" varStatus="s">
							<c:if test="${s.index % 2 == 0}">
								<tr style="background-color: bule">
									...
									<td width="200px"><c:if test="${u.c_state eq '待销售'}">
											<a
												href="javascript:if(confirm('确认要注销吗？')); 
									$(location).prop('href', '<%=path%>CancleCardServlet?action=cancle&s_id=${u.c_id}&s_state=${u. c_state}&loginname=${loginname}');"
												id="deletecard">注销</a>
...
<script type="text/javascript">
	$(function() {
		...
		$("#quary_cid").on("click",function(){
			...
			var path ="<%=path%>CancleCardServlet?action=cancle_Main&c_id="+c_id+"&c_id1="+c_id1+"&s_state="+s_state+"&currentPage=1";
...
<script type="text/javascript">
		...
		$("#last").on("click",function(){
			...
			var c_id1 = $("#c_id1").val();
			var s_state = $("#s_state").val();
			var path ="<%=path%>CancleCardServlet?action=cancle_Main&c_id="+c_id+"&c_id1="+c_id1+"&s_state="+s_state+"&currentPage=${currentPages-1 }";
			$(location).prop('href', path);
...
<script type="text/javascript">		
	$(function() {
		$("#end").on("click",function(){
			...
			var path ="<%=path%>
	CancleCardServlet?action=cancle_Main&c_id="
									+ c_id
									+ "&c_id1="
									+ c_id1
									+ "&s_state="
